...
who_is_the_mad_queen(X) :-
    rightful_true_heir_iron_throne_before_concubine_son_screw_everything(X),
    destroy(X, capital),
    mother_dragons(X, _),
    !.
...
who_was_the_biggest_threat_to_daenerys(X) :- 
    ...
    rightful_true_heir_iron_throne_counting_with_concubine_son(X),
    !.
...
% Rightful Heirs to the Iron Throne, by Common Sense and Lineage,
% if the forgot concubine Son didn't screw it all in the last episode
rightful_true_heir_iron_throne_before_concubine_son_screw_everything(X) :-
    % Most preferred by most of people
    descendant(X, aegon_v_targaryen),
    kill(jon_snow, X),
...
% Rightful Heirs to the Iron Throne, by Common Sense and Lineage,
% counting with the forgot concubine Son from the Targaryen and Stark House
rightful_true_heir_iron_throne_counting_with_concubine_son(X) :-
    % Most preferred by most of people
    descendant(X, aegon_v_targaryen),
    status(X, alive),
